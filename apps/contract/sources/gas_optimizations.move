module xylkit::gas_optimizations {
    ...
    public fun sort_drips_configs(configs: vector<DripsConfig>): vector<DripsConfig> {
        ...
        while (i < len) {
            ...
            let j = i;
            while (
                j > 0 && splits::address_less_than(drips::get_drips_config_receiver(&current), drips::get_drips_config_receiver(vector::borrow(&configs, j - 1)))
            ) {
    ...
    public fun sort_splits_receivers(
        ...
        while (i < len) {
            ...
            let j = i;
            while (
                j > 0 && splits::address_less_than(splits::get_splits_receiver_receiver(&current), splits::get_splits_receiver_receiver(vector::borrow(&receivers, j - 1)))
            ) {
    ...
    public fun binary_search_drips_config(
        ...
        while (left <= right) {
            ...
            if (drips::get_drips_config_receiver(config) == receiver) {
                return (true, mid)
            } else if (splits::address_less_than(drips::get_drips_config_receiver(config), receiver)) {
                left = mid + 1;
            } else {
                if (mid == 0) {
                    return (false, mid)
    ...
    /// - receiver: The receiver address to search for
    /// Returns: (bool, u64) - (found, index)
    public fun binary_search_splits_receiver(
        receivers: &vector<SplitsReceiver>,
        receiver: address,
    ): (bool, u64) {
        let len = vector::length(receivers);
        ...
        while (left <= right) {
            ...
            if (splits::get_splits_receiver_receiver(config) == receiver) {
                return (true, mid)
            } else if (splits::address_less_than(splits::get_splits_receiver_receiver(config), receiver)) {
                left = mid + 1;
            } else {
                if (mid == 0) {
                    return (false, mid)
    ...
    /// Update data for DripsConfig batch operations
    struct DripsConfigUpdate has copy, drop {
        receiver: address,
        amt_per_sec: u128,
        start_cycle: u64,
        duration: u64,
    ...
    public fun batch_update_drips_configs(
        ...
        updates: vector<DripsConfigUpdate>,
    ): vector<DripsConfig> {
        ...
        while (i < updates_len) {
            ...
            let amt_per_sec = update.amt_per_sec;
            let start_cycle = update.start_cycle;
            let duration = update.duration;

            // Create a new config
            let new_config = drips::create_drips_config(
                receiver,
                amt_per_sec,
                start_cycle,
                duration,
    ...
    /// Update data for SplitsReceiver batch operations
    struct SplitsReceiverUpdate has copy, drop {
        receiver: address,
        weight: u32,
    }
    ...
    public fun batch_update_splits_receivers(
        ...
        updates: vector<SplitsReceiverUpdate>,
    ): vector<SplitsReceiver> {
        ...
        while (i < updates_len) {
            ...
            let new_receiver = splits::create_splits_receiver(receiver, weight);
    ...
    public fun table_get_with_default<K: copy + drop, V: copy + drop>(
        t: &Table<K, V>,
        key: K,
        default_value: V,
    ): V {
    ...
    public fun calculate_total_weight(receivers: &vector<SplitsReceiver>): u32 {
        ...
        while (i < len) {
            let receiver = vector::borrow(receivers, i);
            total_weight = total_weight + splits::get_splits_receiver_weight(receiver);
            i = i + 1;
        };
    ...
    public fun calculate_total_streamed(configs: &vector<DripsConfig>): u128 {
        ...
        while (i < len) {
            ...
            let stream_amount =
                drips::get_drips_config_amt_per_sec(config) * (drips::get_drips_config_duration(config) as u128);
    ...
    public fun validate_drips_configs(
        sender_addr: address,
        configs: &vector<DripsConfig>,
    ): bool {
        let len = vector::length(configs);
        ...
        if (
            drips::get_drips_config_amt_per_sec(config) == 0 || drips::get_drips_config_duration(config) == 0 || drips::get_drips_config_receiver(config) == sender_addr
        ) {
            return false
        };
        ...
        while (i < len) {
            ...
            if (
                drips::get_drips_config_amt_per_sec(curr_config) == 0 || drips::get_drips_config_duration(curr_config) == 0 || drips::get_drips_config_receiver(curr_config) == sender_addr
            ) {
                return false
            };
            ...
            if (
                !splits::address_less_than(
                    drips::get_drips_config_receiver(prev_config),
                    drips::get_drips_config_receiver(curr_config),
                )
            ) {
                return false
            };
    ...
    public fun validate_splits_receivers(
        ...
        if (
            splits::get_splits_receiver_weight(receiver) == 0 || splits::get_splits_receiver_receiver(receiver) == sender_addr
        ) {
            return false
        };
        total_weight = total_weight + splits::get_splits_receiver_weight(receiver);
        ...
        while (i < len) {
            ...
            if (
                splits::get_splits_receiver_weight(curr_receiver) == 0 || splits::get_splits_receiver_receiver(curr_receiver) == sender_addr
            ) {
                return false
            };
            ...
            if (
                !splits::address_less_than(
                    splits::get_splits_receiver_receiver(prev_receiver),
                    splits::get_splits_receiver_receiver(curr_receiver),
                )
            ) {
                return false
            };
            ...
            // Add to total weight
            total_weight = total_weight + splits::get_splits_receiver_weight(curr_receiver);
    ...
    /// Balance entry for compact storage
    struct BalanceEntry has copy, drop {
        address: address,
        balance: u128,
    }

    /// Create a new BalanceEntry
    public fun create_balance_entry(address: address, balance: u128): BalanceEntry {
        BalanceEntry { address, balance }
    }
    ...
    /// Returns: A vector of BalanceEntry
    public fun create_compact_balances(
        addresses: vector<address>,
        balances: vector<u128>,
    ): vector<BalanceEntry> {
        let result = vector::empty<BalanceEntry>();
        ...
        while (i < len) {
            ...
            if (balance > 0) {
                vector::push_back(&mut result, BalanceEntry { address: addr, balance });
            };
