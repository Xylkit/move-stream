...
/**
 * Format account ID - show wallet address if available, otherwise truncated ID
 */
export function formatAccountId(accountId: string, length = 10): string {
  const wallet = extractWalletAddress(accountId);
  if (wallet) {
...
/**
 * Get linkable address from account ID
 */
export function getAccountAddress(accountId: string): string {
  return extractWalletAddress(accountId) || accountId;
}
