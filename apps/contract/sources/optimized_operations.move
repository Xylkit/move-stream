module xylkit::optimized_operations {
    ...
    /// Error codes
    const CATEGORY: u8 = 0; // CATEGORY_GENERAL
    ...
    public fun optimized_set_streams<CoinType>(
        ...
        let new_configs =
            gas_optimizations::batch_update_drips_configs(current_configs, updates);
    ...
    /// Parameters:
    /// - sender: The signer of the transaction
    /// - updates: A vector of SplitsReceiverUpdate
    public fun optimized_set_splits(
        sender: &signer, updates: vector<gas_optimizations::SplitsReceiverUpdate>
    ) {
        let sender_addr = signer::address_of(sender);
        ...
        // Apply the updates in batch
        let new_receivers =
            gas_optimizations::batch_update_splits_receivers(current_receivers, updates);
    ...
    public fun optimized_collect<CoinType>(
        sender: &signer, account_ids: vector<address>, transfer_to: address
    ): u128 {
        let sender_addr = signer::address_of(sender);
        ...
        while (i < len) {
            ...
            // Collect from splits
            let splits_amount = 0u128; // Placeholder - in real implementation, would collect from splits
    ...
    public fun optimized_give<CoinType>(
        sender: &signer, receivers: vector<address>, amounts: vector<u64>
    ): u64 {
    ...
    /// Returns: A vector of gas_optimizations::BalanceEntry
    public fun optimized_receive_streams<CoinType>(
        account_ids: vector<address>, max_cycles: u32
    ): vector<gas_optimizations::BalanceEntry> {
        let result = vector::empty<gas_optimizations::BalanceEntry>();
        let len = vector::length(&account_ids);
        let i = 0;
        ...
        while (i < len) {
            ...
            if (amount > 0) {
                vector::push_back(
                    &mut result,
                    gas_optimizations::create_balance_entry(account_id, amount)
                );
            };
    ...
    /// Split result entry for optimized operations
    struct SplitResult has drop, copy {
        account_id: address,
        split_amount: u128,
    ...
    /// Returns: A vector of SplitResult
    public fun optimized_split<CoinType>(account_ids: vector<address>): vector<SplitResult> {
        let result = vector::empty<SplitResult>();
        let len = vector::length(&account_ids);
        let i = 0;
        ...
        while (i < len) {
            ...
            if (splits::exists_store(account_id)) {
                ...
                let (split_amount, collected_amount) =
                    splits::split<CoinType>(account_id, receivers);

                // Only add non-zero amounts to the result
                if (split_amount > 0) {
                    vector::push_back(
                        &mut result,
                        SplitResult { account_id, split_amount, collected_amount }
                    );
                };
            };
    ...
    public fun optimized_calculate_streamed_amount(
        config: &DripsConfig, current_cycle: u64, max_cycle: u64
    ): u128 {
        // If the stream hasn't started yet, return 0
        if (current_cycle < drips::get_drips_config_start_cycle(config)) {
            return 0
        };

        // Calculate the end cycle of the stream
        let seconds_per_cycle = drips::get_seconds_per_cycle();
        let stream_cycles = drips::get_drips_config_duration(config)
            / seconds_per_cycle;
        if (drips::get_drips_config_duration(config) % seconds_per_cycle > 0) {
            stream_cycles = stream_cycles + 1;
        };
        let end_cycle = drips::get_drips_config_start_cycle(config) + stream_cycles;
        ...
        if (calc_end_cycle <= current_cycle) {
            return drips::get_drips_config_amt_per_sec(config)
                * (drips::get_drips_config_duration(config) as u128)
        };

        // Calculate how many cycles have passed since the stream started
        let cycles_passed = current_cycle - drips::get_drips_config_start_cycle(config);

        // Calculate the amount streamed so far using the optimized function
        let seconds_passed = cycles_passed * seconds_per_cycle;
        if (seconds_passed > drips::get_drips_config_duration(config)) {
            seconds_passed = drips::get_drips_config_duration(config);
        };

        gas_optimizations::calculate_streamed_amount_optimized(
            drips::get_drips_config_amt_per_sec(config),
            drips::get_drips_config_duration(config),
            seconds_passed
        )
    }
    ...
    public fun optimized_validate_configs(
        sender_addr: address, configs: &vector<DripsConfig>
    ) {
        ...
        error_handling::assert_with_error(is_valid, CATEGORY, E_INVALID_ARGUMENT);
    }
    ...
    public fun optimized_validate_receivers(
        sender_addr: address, receivers: &vector<SplitsReceiver>
    ) {
        // Use the optimized validation function from gas_optimizations
        let is_valid =
            gas_optimizations::validate_splits_receivers(
                sender_addr,
                receivers,
                splits::get_total_splits_weight()
            );
    ...
    public fun optimized_current_cycle(): u64 {
        ...
        current_timestamp / drips::get_seconds_per_cycle()
    }

    /// Stream entry for optimized operations
    struct StreamEntry has drop, copy {
        sender: address,
        amt_per_sec: u128,
        start_cycle: u64,
        end_cycle: u64
    ...
    /// Returns: A vector of StreamEntry
    public fun optimized_find_receivable_streams(
        receiver: address, configs: &vector<DripsConfig>
    ): vector<StreamEntry> {
        let result = vector::empty<StreamEntry>();

        // Use binary search to find the receiver
        let (found, index) =
            gas_optimizations::binary_search_drips_config(configs, receiver);
        ...
        if (found) {
            ...
            let seconds_per_cycle = drips::get_seconds_per_cycle();
            let stream_cycles =
                drips::get_drips_config_duration(config) / seconds_per_cycle;
            if (drips::get_drips_config_duration(config) % seconds_per_cycle > 0) {
                stream_cycles = stream_cycles + 1;
            };
            let end_cycle = drips::get_drips_config_start_cycle(config) + stream_cycles;
            ...
            vector::push_back(
                &mut result,
                StreamEntry {
                    sender: drips::get_drips_config_receiver(config),
                    amt_per_sec: drips::get_drips_config_amt_per_sec(config),
                    start_cycle: drips::get_drips_config_start_cycle(config),
                    end_cycle
                }
            );
        };
    ...
    public fun optimized_batch_operations<CoinType>(
        ...
        let total_collected =
            gas_optimizations::batch_collect<CoinType>(sender, collect_from);
    ...
    public fun optimized_calculate_total_streamed(
    ...
    public fun optimized_calculate_total_weight(
    ...
    public fun optimized_deduplicate_addresses(
    ...
    public fun optimized_table_get<K: copy + drop, V: copy + drop>(
        t: &table::Table<K, V>, key: K, default_value: V
    ): V {
    ...
    public fun optimized_calculate_cycle_boundaries(cycle: u64): (u64, u64) {
        gas_optimizations::calculate_cycle_boundaries(
            cycle, drips::get_seconds_per_cycle()
        )
    }
    ...
    public fun optimized_calculate_cycles_between(
        start_timestamp: u64, end_timestamp: u64
    ): u64 {
        gas_optimizations::calculate_cycles_between(
            start_timestamp, end_timestamp, drips::get_seconds_per_cycle()
        )
    }
}
