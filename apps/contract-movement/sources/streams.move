module xylkstream::streams {
    ...
    /// Maximum u64 value (used instead of Move 2.3's MAX_U64 builtin)
    ...
    /// Global storage for all streams state, stored at @xylkstream
    ...
    /// Initialize streams storage at @xylkstream with configurable cycle length
    ///
    ...
    fun get_cycle_secs(): u64 acquires StreamsStorage {
        borrow_global<StreamsStorage>(@xylkstream).cycle_secs
    }

    /// Returns the minimum amt_per_sec for this deployment
    fun get_min_amt_per_sec(): u256 acquires StreamsStorage {
        borrow_global<StreamsStorage>(@xylkstream).min_amt_per_sec
    }
    ...
    fun process_cycles(
        ...
        while (cycle < to_cycle) {
            if (state.amt_deltas.contains(cycle)) {
                ...
                acc_received += i128::as_u128(&acc_rate);
    ...
    fun update_receiver_states(
        ...
        loop {
            ...
            if (pick_curr && pick_new) {
                ...
                let amt_per_sec = i256::from_u128((curr_recv.config.amt_per_sec as u128));
                let neg_amt_per_sec = i256::neg(&amt_per_sec);
                add_delta_range(state, start, end, neg_amt_per_sec, cycle_secs);
