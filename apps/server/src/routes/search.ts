...
async function findDeploymentStartVersion(address: string): Promise<string> {
  try {
    ...
    const txUrl = `${getRpcUrl()}/accounts/${address}/transactions?start=0&limit=${Math.min(totalTxs, 1000)}`;
    ...
    console.log(`[Search] Deployment first tx at ${oldestVersion}, starting cursor at ${start}`);
    return start;
  } catch (err) {
...
router.get('/', async (req, res, next) => {
  try {
    ...
    if (q.length !== 66) {
      return res.status(400).json({ error: 'Address must be 66 characters (0x + 64 hex digits)' });
    }
    ...
    if (isDeployment) {
      ...
      await db
        ...
        .values({ address: q, network: 'movement-testnet', first_seen_at: new Date().toISOString(), last_tx_version: null })
    ...
    try {
      ...
      if (txRes.ok) {
        ...
        await db
          ...
          .onConflict((oc) => oc.columns(['deployment_address', 'event_type'])
            .doUpdateSet({ 
              last_sequence: lastSequence.toString(),
              updated_at: new Date().toISOString() 
            }))
    ...
    console.log(`[Search] User ${q} batch: ${startSequence}-${lastSequence}, found ${discoveredDeployments.size} deployment(s), hasMore: ${hasMore}`);
    
    // Register discovered deployments
    for (const depAddr of discoveredDeployments) {
      // Use binary search to find deployment version
      const deploymentVersion = await findDeploymentVersion(depAddr);
      
      await db
        .insertInto('deployments')
        .values({ address: depAddr, network: 'movement-testnet', first_seen_at: new Date().toISOString() })
        .onConflict((oc) => oc.column('address').doNothing())
        .execute();
