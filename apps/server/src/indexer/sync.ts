...
const EVENT_TYPES = ['StreamsSet', 'SplitsSet', 'Given', 'Received', 'Squeezed', 'SplitExecuted', 'Collected'];
...
async function canSync(deployment: string): Promise<{ allowed: boolean; lastSyncedAt: string | null; nextAvailable: string }> {
  const db = getDb();
  const meta = await db
  ...
  if (meta.has_more) {
    return { allowed: true, lastSyncedAt: meta.last_synced_at, nextAvailable: new Date().toISOString() };
  }
 ...
 */
async function fetchEventsFromTransactions(deployment: string, startVersion: string, limit: number): Promise<{ events: MovementEvent[]; lastVersion: string; transactionsFetched: number }> {
...
export async function syncDeployment(options: SyncOptions): Promise<SyncResult> {
  ...
  if (parseInt(startVersion) >= chainTip) {
    ...
    await db
      ...
      .values({ deployment_address: deployment, last_synced_at: now, events_processed: 0, sync_duration_ms: 0, has_more: 0 })
      .onConflict((oc) => oc.column('deployment_address').doUpdateSet({ last_synced_at: now, events_processed: 0, sync_duration_ms: 0, has_more: 0 }))
      .execute();
  ...
  const { events, lastVersion, transactionsFetched } = await fetchEventsFromTransactions(deployment, startVersion, limit);
  
  let totalProcessed = 0;
  for (const event of events) {
    if (options.accountId) {
      const data = event.data as Record<string, unknown>;
      const ids = [data.account_id, data.sender_id, data.receiver_id].filter(Boolean).map(String);
  ...
  if (lastVersion !== startVersion) {
    await db
      ...
      .values({ deployment_address: deployment, event_type: 'transactions', last_sequence: lastVersion, updated_at: new Date().toISOString() })
      .onConflict((oc) => oc.columns(['deployment_address', 'event_type']).doUpdateSet({ last_sequence: lastVersion, updated_at: new Date().toISOString() }))
      .execute();
  }
  ...
  await db
    .insertInto('sync_metadata')
    .values({ deployment_address: deployment, last_synced_at: now, events_processed: totalProcessed, sync_duration_ms: syncDuration, has_more: hasMore ? 1 : 0 })
    .onConflict((oc) => oc.column('deployment_address').doUpdateSet({ last_synced_at: now, events_processed: totalProcessed, sync_duration_ms: syncDuration, has_more: hasMore ? 1 : 0 }))
    .execute();
...
async function ensureDeployment(address: string): Promise<void> {
  const db = getDb();
  const existing = await db.selectFrom('deployments').select('address').where('address', '=', address).executeTakeFirst();
  if (!existing) {
    await db.insertInto('deployments').values({ address, network: 'movement-testnet', first_seen_at: new Date().toISOString() }).execute();
  }
}
...
export async function syncAll(force = false): Promise<SyncResult[]> {
  ...
  return Promise.all(deployments.map(dep => syncDeployment({ deployment: dep.address, force })));
}

export async function runSync(options: { deployment?: string; user?: string; force?: boolean; limit?: number } = {}): Promise<SyncResult> {
  const db = getDb();
  ...
  return syncDeployment({ deployment, accountId, force: options.force, limit: options.limit });
}

export async function getSyncStatus(deployment?: string): Promise<{ deployment: string; lastSyncedAt: string | null; ageMs: number }[]> {
  const db = getDb();
  
  let query = db.selectFrom('sync_metadata').select(['deployment_address', 'last_synced_at', 'events_processed', 'sync_duration_ms']);
  if (deployment) {
